        ...
        if char == '"':
            ...
            if char == "^":
                ...
                if (
                    ...
                    or len(language) > 20
                    or len(language) < 2
                    or ("-" in language and len(language[language.index("-") :]) > 9)
                ):
                    if TEST:
            ...
            while True:
                ...
                if not term:
                    ...
                    if term == "[":
                        ...
                        yield from triplesFromTerms(
                            generator, predicates, givenSubject=term
                        )
                    else:
                        yield (listNode, "rdf:first", term)
...
def triplesFromTurtleFile(file, message=None, predicates=None):
    ...
    with open(file, "rb") as reader:
        yield from triplesFromTerms(
            termsAndSeparators(charGenerator(byteGenerator(reader))), predicates
        )
    if message:
...
def visitWikidataEntities(args):
    ...
    with open(file, "rb", buffering=1 * mega) as wikidataReader:
        ...
        for graph in entitiesFromTriples(
            triplesFromTerms(
                termsAndSeparators(charGenerator(byteGenerator(wikidataReader)))
            )
        ):
            visitor.visit(graph)
